...
\paragraph{Signs encode angles.} For a random hyperplane with normal $g \sim \mathcal{N}(0, I_d)$, the probability that two vectors fall on opposite sides is exactly proportional to the angle between them (the Goemans--Williamson identity):
\begin{equation}
...
\end{equation}
Equivalently, sign-quantizing both vectors gives $\mathbb{E}\bigl[\tfrac1k \operatorname{sign}(Rx)^\top \operatorname{sign}(Ry)\bigr] = 1 - \tfrac{2}{\pi}\theta(x,y)$, so the dot product of sign codes is a monotone function of the angle and recovers cosine ordering. The quantized-JL (QJL) refinement keeps the query in full precision to cut variance. Using the Gaussian identity $\mathbb{E}\,[\,g\,\operatorname{sign}(g^\top x)] = \sqrt{2/\pi}\,x/\|x\|$, the estimator
\begin{equation}
...
\paragraph{Error scales as $4^{-b}$.} Modelling the rounding error of each coordinate as uniform on $[-\Delta/2, \Delta/2]$ gives the standard quantization-noise variance
\begin{equation}
