...
\paragraph{Why these simplifications are defensible here.} TurboQuant's most intricate machinery targets two conditions our study does not face: adversarial worst-case inputs, and applications such as KV-cache attention where the absolute value of each inner product feeds a softmax. Our setting is gentler, a fixed corpus of real embeddings that are already close to isotropic and scored by relative rank, and that gap is what each simplification trades on. The cheap signed-permutation rotation avoids the $O(d^2)$ cost of a dense rotation, whose job (spreading concentrated energy) is largely already done by the embedding model. Uniform quantization in place of Max--Lloyd codebooks costs a fraction of a bit on near-Gaussian data, and it buys experimental control: the plain \texttt{scalar} baseline uses the identical quantizer, so any measured TurboQuant gain is attributable to the residual stage alone. Taking residual signs directly ($S = I$) forgoes the unbiasedness guarantee, which matters for absolute-magnitude attention but not for ranked retrieval, and our results below confirm the stripped-down correction still recovers most of the low-bit benefit (76\% versus 63\% at matched Stage-1 bits). Finally, we label configurations by Stage-1 bits and add the residual bit on top rather than carving it out; since every figure reports \emph{measured} bits per dimension, this changes no comparison. The consequence is honesty about bits: our \texttt{turboquant\_2bit} actually consumes about $3.2$ bits per dimension (two Stage-1 bits, one residual bit, plus amortized rotation and norm metadata), not the nominal $2.0$.

\section{Methods and experimental setup}

\subsection{Corpus and embeddings}

The retrieval corpus is the source text of Professor He Wang's MATH 5110 textbook, \emph{Advanced Linear Algebra for AI}, pulled directly from its public Quarto (\texttt{.qmd}) repository at \href{https://github.com/wanghemath/Book-AdvancedLinearAlgebraAI}{\texttt{github.com/wanghemath/Book-AdvancedLinearAlgebraAI}}. Chapters are split on section boundaries and then into overlapping character windows (900 characters per chunk, 120-character overlap), yielding \textbf{1{,}380 chunks}. Each chunk is embedded with \texttt{text-embedding-3-small} at $d = 256$. Retrieval is ranked by \textbf{cosine similarity} throughout (the scoring functions normalize vectors at comparison time), so ranking depends on direction, not magnitude.
