...
TurboQuant (Zandieh et al., ICLR 2026) is a \emph{data-oblivious} quantizer that is provably near-optimal for both mean-squared-error (MSE) and \emph{inner-product} distortion. Its starting observation is that an MSE-optimal quantizer is a \emph{biased} inner-product estimator --- minimizing reconstruction error and preserving dot products are different objectives --- so a single quantizer cannot serve retrieval well. TurboQuant bridges the gap with a two-stage construction. For a target budget of $b$ bits per coordinate:
\begin{itemize}
  \item \textbf{Stage 0 --- Rotate.} Each vector is multiplied by a random rotation $\Pi$ (e.g. the $Q$ factor of a random Gaussian matrix). For a vector on the unit sphere this makes every coordinate follow the same (scaled) Beta distribution --- converging to $\mathcal{N}(0, 1/d)$ in high dimension --- and renders distinct coordinates nearly independent, so each can be quantized on its own.
  \item \textbf{Stage 1 --- MSE-optimal scalar quantization ($b-1$ bits).} Because the per-coordinate distribution is known in closed form, TurboQuant precomputes the \textbf{Max--Lloyd} optimal scalar quantizer for that Beta distribution (a continuous 1-D $k$-means problem) and applies it to each rotated coordinate. This minimizes the $L_2$ norm of the residual $r = x - \hat{x}_1$ but, alone, biases inner products.
...
\begin{equation}
\operatorname{score}(q, x) \;=\; q^\top \hat{x}_1 \;+\; \frac{\sqrt{\pi/2}}{d}\,(Sq)^\top \operatorname{sign}(S r),
\end{equation}
which TurboQuant proves is unbiased ($\mathbb{E}[\,\cdot\,] = \langle q, x\rangle$) with near-optimal variance across all bit-widths. Note the bit accounting: a budget of $b$ buys $b-1$ Stage-1 bits \emph{plus} the 1-bit residual --- the residual is carved out of the budget, not added on top.

\paragraph{How it works.} TurboQuant is best read as a correction layer bolted onto scalar quantization. The rotation (Stage 0) destroys any coordinate system in which energy is concentrated on a few axes and gives every coordinate the same known distribution, so a per-coordinate scalar quantizer is both optimal and cheap. Stage 1 then leaves behind a structured error --- the same correlated, direction-biasing residual that hurts plain scalar quantization at low bit budgets. The insight is that this residual, while not negligible, is \emph{small in norm and roughly isotropic} after rotation, so it is exactly the kind of vector a 1-bit QJL sketch estimates well \emph{without bias}. Spending one bit per coordinate on the residual's inner-product contribution removes most of the Stage-1 bias without storing the residual itself. The payoff is largest precisely where scalar quantization is weakest (2--3 bits) and shrinks once the Stage-1 quantizer is already accurate (4+ bits), where the rotation and residual become overhead.

\paragraph{Our pedagogical implementation, and why the simplifications are defensible here.} TurboQuant's most intricate machinery is engineered for two conditions our study does not face: \emph{adversarial worst-case inputs}, and applications such as KV-cache attention where the \emph{absolute} value of each inner product is fed through a softmax. Our setting is gentler --- a fixed corpus of real embeddings that are already close to isotropic, scored by \emph{relative} rank --- and that gap is exactly what each simplification trades on.
\begin{itemize}
  \item \textbf{Cheap rotation instead of a dense one.} A full $d \times d$ random rotation costs $O(d^2)$ time and storage per use, which is why even the production method uses fast structured transforms rather than a literal Haar matrix. The rotation's theoretical role is to rescue worst-case inputs whose energy piles onto a few coordinates, but \texttt{text-embedding-3-small} vectors are already near-isotropic, so most of that work is done before we begin. An $O(d)$ permutation-plus-sign stand-in keeps the cost negligible; the residual stage then absorbs whatever decorrelation it fails to achieve.
  \item \textbf{Uniform quantization instead of Max--Lloyd codebooks.} Max--Lloyd is optimal only in the asymptotic, exactly-Beta limit, and its measured edge over uniform quantization on near-Gaussian sources is a fraction of a bit. The decisive reason, though, is experimental control: our plain \texttt{scalar} baseline uses the \emph{identical} uniform quantizer, so any gain we attribute to TurboQuant comes provably from the residual-correction stage and not from a better Stage-1 codebook --- the one variable we want to isolate.
  \item \textbf{Residual signs instead of the full QJL projection.} The random sketch $S$ and the $\sqrt{\pi/2}$ factor exist to make the residual estimate \emph{unbiased with dimension-free variance for any adversarial query direction} --- a guarantee that matters when the raw inner-product magnitude drives an attention softmax. Retrieval, by contrast, only needs the correction to nudge close neighbors into the right order, and we already retain the dominant magnitude term, the per-vector residual norm $\|r\|$. Dropping $S$ avoids another $O(d^2)$ transform and folds the remaining calibration into a single $1/\sqrt{d}$ weight; our results below confirm empirically that this stripped-down correction still recovers most of the low-bit benefit (76\% vs.\ 63\% at matched Stage-1 bits).
  \item \textbf{Labelling by Stage-1 bits.} We name configurations by Stage-1 fidelity and add the residual bit on top rather than carving it from the budget. Because every figure and table reports \emph{measured} bits per dimension, this convention changes no comparison --- it only keeps the bit ladder legible.
\end{itemize}
The mechanism under study --- a coarse first stage corrected by a 1-bit residual --- is therefore faithful; what we trade away (provable optimality and unbiasedness) is precisely the part that buys worst-case and absolute-magnitude guarantees we do not need for ranked retrieval.

The consequence is honesty about bits: our \texttt{turboquant\_2bit} actually consumes about $3.2$ bits per dimension (two Stage-1 bits, one residual bit, plus amortized rotation and norm metadata), not the nominal $2.0$. This distinction is essential when comparing methods.
