...
\paragraph{Our pedagogical implementation.} We keep TurboQuant's two-stage skeleton but simplify each stage so the code stays short and dependency-free, at the cost of the paper's optimality and unbiasedness guarantees. We replace the dense random rotation with a cheap $O(d)$ coordinate permutation plus Rademacher signs; we use \emph{uniform} scalar quantization in place of the Beta-distribution Max--Lloyd codebooks; and we take the sign of the residual directly (i.e. $S = I$, dropping the QJL random projection and the $\sqrt{\pi/2}$ debiasing constant), scoring with $\|r\|\,\operatorname{sign}(r)^\top q / \sqrt{d}$. We also \emph{add} the residual bit on top of the Stage-1 budget rather than carving it out, so our \texttt{turboquant\_$b$bit} spends $b$ Stage-1 bits plus one residual bit. The mechanism under study --- a coarse first stage corrected by a 1-bit residual --- is faithful even though the constants are not.
